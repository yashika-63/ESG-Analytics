...
const WasteAnalytics = ({ filters }: { filters?: FilterOptions }) => {
  ...
  const loadDataFromAPI = async () => {
    ...
    try {
      ...
      console.error('Waste data API error:', error);
      setLoadStatus('error');
    } finally {
      setIsLoading(false);
    }
  ...
  const analytics = useMemo(() => {
    ...
    // Group by SubCategory (disposal methods)
    ...
    return {
      ...
      overview: {
        totalQuantity,
        totalValue,
        avgQuantity: wasteData.length > 0 ? totalQuantity / wasteData.length : 0,
        avgValue: wasteData.length > 0 ? totalValue / wasteData.length : 0
      }
    };
  }, [wasteData]);
      ...
      {wasteData.length > 0 && (
        <Tabs defaultValue="overview" className="space-y-4">
          <TabsList className="grid w-full grid-cols-5">
            ...
            <TabsTrigger value="parameter">Waste Types</TabsTrigger>
            <TabsTrigger value="subcategory">Disposal Methods</TabsTrigger>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={analytics.byParameter}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="totalQuantity" fill={colors[0]} name="Total Quantity (MT)" />
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={analytics.bySubCategory}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="totalQuantity" fill={colors[2]} name="Total Quantity (MT)" />
          ...
          <TabsContent value="plant">
            <Card>
              <CardHeader>
                <CardTitle>Waste Generation by Plant</CardTitle>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                  <RechartsPieChart>
          ...
          <TabsContent value="monthly">
            <Card>
              <CardHeader>
                <CardTitle>Monthly Waste Generation Trend</CardTitle>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
